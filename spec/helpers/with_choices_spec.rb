...
describe WithBreadcrumbs do
  ...
  context 'exercise width choices' do
    ...
    let(:content) { '0:1' }
    it { expect(checked? checked_content, content).to be_true }
    it { expect(checked? unchecked_content, content).to be_false }
    it { expect(checked? unchecked_content, '').to be_false }
  end
end
