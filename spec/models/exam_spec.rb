...
describe Exam do
  ...
  describe '#upsert' do
    ...
    let(:exam_json) { {eid: '1', slug: guide.slug, start_time: 5.minutes.ago, end_time: 10.minutes.since, duration: 150, language: 'haskell', name: 'foo', uids: [], tenant: 'test'} }
    let!(:exam) { Exam.import_from_json! exam_json }
    context 'when new exam and the guide is the same' do
      let(:guide2) { create(:guide) }
      let(:exam_json2) { {eid: '2', slug: guide.slug, start_time: 5.minutes.ago, end_time: 10.minutes.since, duration: 150, language: 'haskell', name: 'foo', uids: [], tenant: 'test'} }
  ...
  describe '#access!' do
    ...
    context 'enabled' do
      ...
      context 'import_from_json' do
        ...
        let(:guide) { create(:guide) }
        let(:duration) { 150 }
        let(:exam_json) { {eid: '1', slug: guide.slug, start_time: 5.minutes.ago, end_time: 10.minutes.since, duration: duration, language: 'haskell', name: 'foo', uids: [user.uid], tenant: 'test'} }
        ...
        context 'existing exam' do
          let(:exam_json2) { {eid: '1', slug: guide.slug, start_time: 5.minutes.ago, end_time: 10.minutes.since, duration: 150, language: 'haskell', name: 'foo', uids: [user2.uid], tenant: 'test'} }
      ...
      context 'update exam does not change user started_at' do
        let(:user) { create(:user, uid: 'auth0|1') }
        let(:guide) { create(:guide) }
        let(:exam_json) { {eid: '1', slug: guide.slug, start_time: 5.minutes.ago, end_time: 10.minutes.since, duration: 150, language: 'haskell', name: 'foo', uids: [user.uid], tenant: 'test'} }
        let(:exam) { Exam.import_from_json! exam_json }
        before { exam.start! user }
